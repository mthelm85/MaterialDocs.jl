...
function _handle_request(sock::TCPSocket, build_dir::String,
    ...
    try
        ...
        # Special route: the color scheme, generated by the same Julia code
        # the build uses. The panel has no color engine of its own.
        if path == "/__scheme__"
...
Generate the color scheme the *build* would produce, as JSON, from the
...
function _editor_panel_js(theme::ThemeConfig)::String
    ...
    (function() {
      ...
      // ── Scheme generation ──
      // There is deliberately no color engine here. The panel asks the Julia
